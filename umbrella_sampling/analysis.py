...
def truncate_data(truncation_time, free_energy_step, restraint_type, sampling_time=None, dof=None, run_number=1):
    ...
    free_energy_step  : str
        Section of the thermodynamic cycle where US simulation is performed
        Options are:
        - "separation" : radial BRD4-DCAF16 separation
    ...
    dof : str
        ...
        - BRD4_only (bound)
        - DCAF16_only (bound)
        - BRD4withDCAF16 (bound)
        - DCAF16withBRD4 (bound)
...
def obtain_RED_value(CV_value, free_energy_step, restraint_type, dof=None, sampling_time=None, run_number=1):
    """
    Generate the metadata file required for WHAM implementation for a specific umbrella sampling run
...
def standard_state_correction(r_star, theta_a_min, theta_b_min, k_boresch):
    """
    Calculate the free energy contribution from release of the Boresch restraints
    to the standard state surface area
    """
...
def SepContribution(x, pmf, r_star):
    """
    Calculate the integral corresponding to the separation free energy contributionn.
    Code adapted from https://github.com/fhh2626/BFEE2
    """
